...
import { getCursorAdvancedName, getCursorDisplayName } from '@utils/cursor-labels';
...
export const CursorCard: React.FC<Props> = (props) => {
  ...
  const displayName = getCursorDisplayName(name);
  const advancedName = getCursorAdvancedName(name);
  ...
  const frameConfig = DELAYS[delayX];
  const frameStep = isAnimated
    ? Math.max(1, Math.ceil(urls.length / frameConfig.frames))
    : 1;
  const expectedFrameCount = isAnimated ? Math.ceil(urls.length / frameStep) : 1;
  ...
  const fetchSvg = async (signal: AbortSignal) => {
    ...
    try {
      const loadSvgAt = async (url: string) => {
        ...
        return svgToPng(svgData);
      };

      const sampledUrls = urls.filter((_, index) =>
        isAnimated ? index % frameStep === 0 : index === 0
      );

      const firstFrame = await loadSvgAt(sampledUrls[0]);
      ...
      if (sampledUrls.length <= 1) {
        return;
      }

      const restFrames: string[] = [];
      for (let i = 1; i < sampledUrls.length; i++) {
        const frame = await loadSvgAt(sampledUrls[i]);
        if (signal.aborted) return;
        if (!frame) {
          break;
        }
        restFrames.push(frame);
      }

      if (restFrames.length > 0) {
        setFrames((current) => [...current, ...restFrames]);
      }
    } catch (e) {
...
Report Issue here: https://github.com/ful1e5/bibata/issues`
  ...
  useEffect(() => {
    if (isAnimated && !loading && frames.length > 1) {
      const intervalId = setInterval(() => {
        setIndex((current) => (current < frames.length - 1 ? current + 1 : 0));
      }, frameConfig.delay);

      return () => clearInterval(intervalId);
    }
  }, [isAnimated, loading, frames.length, frameConfig.delay]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (props.onLoad) {
      props.onLoad(
        { name, frames, delay: frameConfig.delay },
        loading || frames.length === 0 || frames.length !== expectedFrameCount
      );
    }
  }, [loading, frames, expectedFrameCount, frameConfig.delay]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handlePointerLeave = () => {
    ...
    setShowAdvancedName(false);
  };

  return (
    <button
      disabled={!isAnimated}
      aria-label={`${displayName} (${advancedName})`}
      className={
        isAnimated && !loading
      ...
      <div
        ...
        <div title={displayName} className='relative w-full h-24 sm:h-40'>
        ...
        <div className='relative text-center text-[8px] sm:text-sm'>
          {showAdvancedName && (
            <div className='pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[90%] -translate-x-1/2 rounded-xl border border-white/[.12] bg-black/85 px-3 py-2 text-[9px] sm:text-[11px] text-white shadow-xl backdrop-blur-md'>
              <p className='mb-1 uppercase tracking-[0.18em] text-white/45'>
                Advanced Name
              </p>
              <p className='font-mono text-white/90'>{advancedName}</p>
            </div>
          )}
          <p className='py-3'>{displayName}</p>
        </div>
      </div>
    </button>
  );
