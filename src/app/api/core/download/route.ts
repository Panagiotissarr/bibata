...
const isPlatform = (value: string | null): value is 'png' | 'win' | 'x11' =>
...
const configs: Record<string, CursorConfig> = {
  ...
  grabbing: {
    ...
    winname: 'Grabbing',
    xname: 'grabbing',
    links: ['closedhand', 'dnd-move', 'dnd-none', 'fcf21c00b30f7e3f83fe0dfd12e71cff'],
  },
  move: {
    winname: 'Move',
    xname: 'move',
...
export async function GET(request: NextRequest) {
  ...
  try {
    const build = await prisma.cursorBuild.findUnique({
      where: { id: buildId },
    });

    if (!build) {
      return NextResponse.json(
        { id: buildId, error: ['No cursor build found for this session.'] },
        { status: 404 },
      );
    }

    const config = build.mode === 'right'
      ? rconfigs[build.name] || configs[build.name]
      : configs[build.name];

    if (!config) {
      return NextResponse.json(
        { id: buildId, error: [`Unable to find configuration for '${build.name}'`] },
        { status: 400 },
      );
    }

    const frames = build.frames.map((f) => dataUrlToBytes(f));
    ...
    if (platform === 'png') {
      if (frames.length === 1) {
        ...
        archiveFiles.push({ name: `${build.name}.png`, data: pngData });
      } else {
        ...
        for (let i = 0; i < frames.length; i++) {
          ...
          archiveFiles.push({ name: `${build.name}-${index}.png`, data: pngData });
        }
      }
    }
    ...
    if (platform === 'x11' && config.xname) {
      ...
      if (config.links) {
        ...
        for (const link of config.links) {
          const base = archiveFiles.find((f) => f.name === baseFile);
          if (base) {
            archiveFiles.push({ name: link, data: base.data });
          }
        }
      }
    }
    ...
    const fileName = `${build.name}-${buildId.slice(0, 5)}-v${version}-${platform}${ext}`;
