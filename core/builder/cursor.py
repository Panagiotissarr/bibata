...
MASTER_CURSOR_SIZE = 256


def scale_hotspot(value: int, size: int) -> int:
    scaled = round((value / MASTER_CURSOR_SIZE) * size)
...
def store_cursors(sid: str, data: UploadFormData, logger: Logger):
    ...
    try:
        ...
        if not config:
            ...
            cur = b""
            cursor_name = ""
            hotspot = (scale_hotspot(config.x, size), scale_hotspot(config.y, size))
            ...
            if platform == "win" and config.winname:
                blob = open_blob(pngs, hotspot, [size], 1)
            ...
            if platform == "x11" and config.xname:
                blob = open_blob(pngs, hotspot, [size], delay)
